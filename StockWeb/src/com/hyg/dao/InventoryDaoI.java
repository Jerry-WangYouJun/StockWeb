package com.hyg.dao;

import java.math.BigDecimal;
import java.sql.SQLException;
import java.util.List;
import java.util.Map;

import com.hyg.bean.GoodsBean;
import com.hyg.bean.InventoryBean;

public interface InventoryDaoI {
	/** 
	 * 添加库存
	 */
	public int insert(InventoryBean inventory) throws SQLException;

	/** 
	 * 修改库存
	 */
	public int update(BigDecimal pk, InventoryBean inventory) throws SQLException ;

	/** 
	 * 删除库存
	 */
	public int delete(BigDecimal pk) throws SQLException ;

	/** 
	 * 根据主键ID列查询
	 */
	public InventoryBean findByPrimaryKey(BigDecimal id) throws SQLException ;

	/** 
	 *查询所有库存数据
	 */
	public InventoryBean[] findAll() throws SQLException ;

	/** 
	 * 动态列查询
	 */
	public InventoryBean[] findByDynamicSelect(String sql, Object[] sqlParams) throws SQLException ;

	/** 
	 * where条件查询，注意每个查询条件都要以 and 或or 逻辑修饰符开头
	 */
	public InventoryBean[] findByDynamicWhere(String sql, Object[] sqlParams) throws SQLException ;

	/**
	 * 将Map对象转成Object对象
	 * @param list
	 * @return
	 */
	public InventoryBean[] MapToObject(List<Map<String, Object>> list);
	/**
	 * 分页进行动态条件查询
	 * @param whereSql 条件sql
	 * @param params   sql对应的参数
	 * @param page     当前页码
	 * @param rows     每页显示行数
	 * @return         库存列表数组
	 * @throws SQLException 
	 */
	public InventoryBean[] findByDynamicWhereByPage(String whereSql, Object[] params,
			int page, int rows) throws SQLException;
	/**
	 * 分页动态列查询
	 * @param sql
	 * @param params
	 * @param page
	 * @param rows
	 * @return
	 * @throws SQLException
	 */
	public InventoryBean[] findByDynamicSelectByPage(String sql, Object[] params,
			int page, int rows) throws SQLException;
	/**
	 * 根据查询条件获得记录总数
	 * @param whereSql  查询条件
	 * @param array
	 * @return
	 * @throws SQLException 
	 */
	public int getTotal(String whereSql, Object[] array) throws SQLException;
	/**
	 * 根据sql查询记录数
	 * @param sql
	 * @param params
	 * @return
	 * @throws SQLException
	 */
	public int getTotalBySql(String sql,Object[] params) throws SQLException;
	/**
	 * 根据SQL查询产品
	 * @param sql
	 * @param array
	 * @return
	 * @throws SQLException 
	 */
	public List<GoodsBean> searchDynamicProducts(String sql, Object[] array) throws SQLException;

}
