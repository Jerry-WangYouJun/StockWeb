...
public class OutstockDetailServiceImpl implements OutstockDetailServiceI {
	private OutstockDetailDaoI detailDao;
	@Override
	public OutStockDetailBean[] findOutstockDetailsByOutstockId(String instockId) {
		// TODO Auto-generated method stub
		
		detailDao = new OutstockDetailDaoImpl();
		String whereSql = " AND OUTSTOCKID=? ";
		Object[] params = new Object[1];
		params[0] =  instockId;
		
		OutStockDetailBean[] detailBeans = null;
		try {
	...
	public JSONObject searchOutstocks(String instockId) {
		...
		OutStockDetailBean[] detailBeans = this.findOutstockDetailsByOutstockId(instockId);
	...
	public JSONObject saveOrUpdate(JSONArray jsonArray, String inStockId,
		...
		if (!"00".equals(inStockState)) {
			json.put("msg", "很抱歉，保存明细信息失败，原因是【当前出库申请单状态不是创建状态】!");
			...
			for (int i = 0; i < jsonArray.size() ; i++) {
				...
				OutStockDetailBean instockDetail = new OutStockDetailBean();
				instockDetail.setOutStockId(inStockId);
	...
	public JSONObject removeDetail(String id) {
		...
		if (StringUtils.isEmpty(id)) {
			...
			try {
				...
				if (count >= 0) {
					json.put("success", true);
					json.put("msg", "恭喜你删除出库单明细成功！");
				}else{
					json.put("success", false);
					json.put("msg", "很抱歉删除出库单明细操作失败，请检查！");
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
